...
- **Status:** Fixed (logic) — `issueRefund` now locks the target payment (`Payment::lockForUpdate()->findOrFail(...)`) for the single-payment case, and locks every candidate payment up front for the order-level case, deriving the refundable total from that same locked set. `processDirectPayment` now locks the order row first (`Order::lockForUpdate()->findOrFail(...)`, must remain the transaction's first query) and recomputes `totalAlreadyPaid` via a fresh query rather than a stale eager-loaded collection. A `CHECK (refunded_amount <= amount)` constraint was added to `payments` (MySQL/MariaDB only, no-op on SQLite) via the migration that created the column. **Out of scope, confirmed with user:** idempotency keys were not added (separate architectural follow-up). `processAutoPayment`/`applyFifo` had the identical unlocked shape and were left unfixed here to keep this ticket's diff focused — subsequently fixed under a follow-up ticket (MUL-33): both now lock the candidate unpaid-order set up front (`Order::...->lockForUpdate()->get()`) and compute each order's amount-owed via a locking `Payment::...->lockForUpdate()->sum(...)` rather than a stale eager-loaded collection, since `applyFifo` runs nested inside `processDirectPayment`'s transaction and can't rely on being the transaction's first plain read the way `processDirectPayment` itself does. True multi-connection concurrency (does the lock actually block a second real transaction, does the CHECK constraint actually reject an out-of-band update) is unverified in this SQLite-only dev/test environment — same caveat as M-04/M-05. Manual verification: against real MySQL, fire two overlapping refund/direct-payment/auto-payment requests for the same fully-refundable payment/order/customer and confirm exactly one succeeds; confirm a manual `UPDATE payments SET refunded_amount = amount + 1` is rejected by the constraint.
...
- **Status:** Fixed (logic) — `SupplierPaymentService::reversePayment` now re-fetches the payment by ID with `lockForUpdate()` inside the transaction and aborts (`InvalidArgumentException` → 422) if the row is already gone; `SupplierPaymentController::destroy` catches it. `SupplierPayment` has no soft deletes/status column, so "already reversed" and "row no longer exists" are the same state — no schema change was needed. Covered by a sequential repeat-delete test and a service-level test that resolves two model instances of the same row (simulating what two concurrent requests' route-model binding would each independently do) and asserts the second `reversePayment()` call throws without writing a duplicate ledger entry. Concurrency behavior under real simultaneous MySQL transactions (does `lockForUpdate()` actually block, per InnoDB semantics) is unverified in this SQLite-only dev/test environment — same caveat as M-04.
