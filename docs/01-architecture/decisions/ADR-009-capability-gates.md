...
- `view-cost-data` — `tenant_admin` and `store_manager`. Governs `cost_price`, all seven `profit_margin*` fields, and supplier pricing.
...
- Capabilities are not nested. `view-cost-data` does not imply `view-reports`: a manager seeing a product's cost is not the same as a manager reading the shop's books.
